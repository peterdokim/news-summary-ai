...
try:
    ...
    from api.config import PRESS_LIST
...
class NewsSummarizer:
    ...
    def extract_news_article(self, url: str) -> Dict[str, str]:
        ...
        try:
            ...
            # 제목 추출 (네이버: #title_area, 다음: .tit_g, 공통: og:title)
            title_elem = (
            ...
            # 본문 추출 (네이버: #dic_area, 다음: .article_view, 공통: og:description)
            article = (
    ...
    def run(
        ...
        if progress_callback:
            progress_callback(0, 'URL 수집 중...')
        try:
            articles = self.crawl_news(keyword, search_engine, max_articles, press_choice, progress_callback)
